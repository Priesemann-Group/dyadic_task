...
def place_ant(ant_idx):
    p = np.random.rand(2) * c.field_size
    is_valid = True  # TODO rename
    while is_valid:  # randomize until no collision to other points.
        is_valid = False
        for i in range(c.ant_amount):
            if not np.isnan(pos[i][0]):  # TODO check in the end if this is necessary
                dist = euclid_dist(pos[i] - p)
                if dist < 2 * c.ant_radius:  # collision with an existing ant
                    p = np.random.rand(2) * c.field_size
                    is_valid = True
                    break
    angle = r.uniform(0., np.pi * 2)
    pos[ant_idx] = p.astype('float64')
    vel[ant_idx] = np.array([np.cos(angle), np.sin(angle)]) * c.velocity
    rad[ant_idx] = c.ant_radius
    if ant_kind[ant_idx] != -1:
        set_share(ant_idx, ant_kind[ant_idx])


shared_0 = 0  # TODO to enum
shared_1 = 1
compet_0 = 2
compet_1 = 3


def set_share(ant_idx, kind):
    if kind == shared_0:
        ant_kind[ant_idx] = shared_0
        shares[ant_idx] = random.randint(0, 2)
    elif kind == shared_1:
        ant_kind[ant_idx] = shared_1
        shares[ant_idx] = random.randint(3, 5)
    elif kind == compet_0:
        ant_kind[ant_idx] = compet_0
        shares[ant_idx] = random.randint(6, 8)
    elif kind == compet_1:
        ant_kind[ant_idx] = compet_1
        shares[ant_idx] = random.randint(9, 11)
...
def spawn_ants():
    ...
    for i in range(c.ant_amount):
        place_ant(i)
    set_share(ant_idx=0, kind=shared_0)
    set_share(ant_idx=1, kind=shared_1)
    set_share(ant_idx=2, kind=compet_0)
    set_share(ant_idx=3, kind=compet_1)


def add_ant(x, y, angle, radius, share=None):  # TODO OLD
    global pos, vel, rad
...
def add_rand_ant(share=None):  # TODO OLD
...
def correct_to_boundaries():
    for i in range(len(pos)):
        if not np.isnan(pos[i][0]):
            for axis in [0, 1]:
                if pos[i][axis] < 0 + rad[i]:
                    pos[i][axis] = 1 + rad[i]
                    vel[i][axis] *= -1
                elif pos[i][axis] > c.field_size[axis] - rad[i]:
                    pos[i][axis] = c.field_size[axis] - 1 - rad[i]
                    vel[i][axis] *= -1
...
def occupied(player_idx):
    ...
    if target_kind == compet_0 or target_kind == compet_1:  # Not a shared target
        ...
        #add_rand_ant()
        place_ant(target_idx[player_idx])  # TODO rename place_ant?
