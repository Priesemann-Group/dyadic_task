...
class GameScheduler(SchedulerBase):
    ...
    def next_game_state(self, player_pos):
        return self._engine.produce_next_game_state(player_pos)
...
class MultiGameScheduler(SchedulerBase):  # TODO rename, this scheduler holds no engine
    def __init__(self, action, send_msg):
        super().__init__(time.time, time.sleep),
        self._action = action
        self._send_msg = send_msg
    ...
    def start(self):
        while True:
            ...
            #data_depositor.new_file()  # TODO
            # TODO reset engine/scores
    ...
    def _reset(self):
        self._next_round = False
        # data_depositor.close()  # delete invalid game record # TODO
        while not self.empty():
            self.cancel(self._scheduled_events.pop())
        self._scheduled_events = []
